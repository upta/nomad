...
- `.claude/skills/` — project-owned skills (`validate-gameplay`, `run-game`, `spacetime-db-reference`) plus two skills symlinked from the validation kit. Run `./setup.ps1` after initializing/updating submodules to (re)create symlinks.
- **agent-skills plugin** — the [agent-skills](https://github.com/addyosmani/agent-skills) workflow skills, the `/spec`, `/plan`, `/code-simplify` commands, and the `code-reviewer`/`security-auditor`/`test-engineer` personas come from a Claude Code plugin (declared in `.claude/settings.json` via `extraKnownMarketplaces` + `enabledPlugins`; accept the install prompt on first session). Its skills are namespaced `agent-skills:<name>`.
- `.claude/commands/` — project-owned commands adapted to this project's validation-first workflow: `/build`, `/test`, `/review`, `/ship`. These override the plugin's same-named generic versions — "tests" here mean in-engine validation scenarios, `/ship` is the Definition-of-Done gate, and `/review`'s security axis is reducer authority, not web OWASP.
