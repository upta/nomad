...
Anti-patterns currently in the codebase — do not copy them; refactor toward scenes when touching these files: `game/Main.cs` builds its `Camera2D` in `_Ready()` and `CreateRemoteNode()` assembles remote-player visuals from raw `Node2D`/`ColorRect` (should be a `RemoteEntity.tscn` instantiated from an `[Export] PackedScene`).
