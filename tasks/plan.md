...
- [x] **Task 5.1: Node framework + map host + hazard system (fire)** — `Ship`-component (`ShipBody`) + `MapHost` refactor (regression-gated), `NodeActivity` state + `SetActiveNode` + the `NodeRules.SeedNode` dispatcher every later node task extends, and the hazard framework with **fire** complete: positioned `Hazard` entities, deterministic spread, proximity damage via the already-declared `DamageType.Fire` (§6.4), walk-up extinguish. **Scope: L** ✅ 49 pure + 51 stdb scenarios green.

- [x] **Task 5.2: Planetside map + airlock + creatures** — Builds the reusable exterior-grid + airlock-transition system on the MapHost seam. Players exit airlocks onto a `PlanetsideMap` to harvest surface nodes (suit required — falls out of existing oxygen rules: exterior = no room → oxygen depletes). Ships **hostile creatures** whole: server-authoritative roaming + contact damage via the already-declared `DamageType.Creature`; avoid/flee, no combat (§6.4, §8). **Scope: L** ✅ 51 pure + 57 stdb green. Airlock zones (`Player.InExterior` + `EnterExterior`/`EnterInterior`), `NodeRules.SeedNode(Planetside)` (surface nodes + crawlers), deterministic `Creature`/`CreatureTick` (chase-nearest/patrol + `DamageType.Creature` contact), MapHost node-switching (QuietMap↔PlanetsideMap on the `NodeActivities` row) + DebugHud "Toggle Node", `TerrainGrid` exterior + `Airlock` walk-up fixtures, `CreatureService`/`CreatureSpawner` client rendering. Play-test feedback folded in: slower creatures (12 px/tick), the airlock reworked to one always-present ship door (not a node-specific two-pad teleporter), and a fix for cloning after a surface death (`RequestRespawn` now clears `InExterior`).

- [x] **Task 5.3: Abandoned wreck salvage** — Reuses the 5.2 airlock/exterior-grid system for a `WreckMap` derelict layout; salvage scattered loot + debris nodes hauled back to cargo (existing storage); reuses 5.1 fire + 5.2 creatures as wreck hazards. **Scope: M–L** ✅ 52 pure + 61 stdb green. `NodeHasExterior(Wreck)`, `NodeRules.SeedNode(Wreck)` (2 WreckageDebris nodes + 3 scattered World loot items — 2 Scrap + 1 rarer Components — + 2 crawler hazards), `ClearTransientNodeState` now also clears uncollected exterior World loot (`DeleteExteriorWorldItems`, outside-hull predicate via `SlotForCell`; interior floor drops persist), `SpawnSalvageLoot` debug reducer. Client `WreckMap.tscn` (dim cold-blue derelict terrain + dark void backdrop, docked ShipBody + airlock); MapHost `SceneForNode`/`OnAirlockUsed`/`AirlockLabel` ("Exit to wreck") extended; DebugHud "Toggle Node" now cycles Quiet→Planetside→Wreck. Salvage haul-back (pick up loot on the derelict → cross the airlock → store in Cargo Bay, puppet sees it) proven end-to-end.
